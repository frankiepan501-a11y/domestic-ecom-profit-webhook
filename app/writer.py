...
async def write_result_sheets(token: str, sm: dict, year_month: str, result: dict,
    ...
    # Finance gate sheets. P0 keeps the same calculation surface as 10_毛利结果表;
    # quarterly consolidation can later append multiple months into 产品毛利_季度.
    await _batch_write(token, sm["产品毛利_月度"], rows10)
    await _batch_write(token, sm["产品毛利_季度"], rows10)
