"""创建新飞书电子表格 + 写 15 sheets + 公式 - v0.2 多店铺多行."""
from datetime import date
from . import feishu, config, settlement_engine
...
HEADERS = {
    ...
    "月度毛利试算": ['月份','平台','店铺','主体','销售订单数','销量','销售额(RMB)','退款(RMB)','净销售额(RMB)','平台费用(RMB)','广告费(RMB)','采购成本(RMB)','物流运费(RMB)','其他费用(RMB)','试算毛利(RMB)','试算毛利率','结算回款/净回款(RMB)','P0缺口数','P1注意项','状态'],
    ...
    "物流匹配明细": ['平台','店铺','月份','订单号','子订单号','运单号','承运商','分摊物流运费','账单文件/API','账单sheet','来源','状态'],
    "费用明细汇总": ['平台','店铺','月份','来源文件','费用类别','订单号','金额','取数字段','备注'],
    "资产分类明细": ['平台','店铺','月份','来源文件','资产类别','订单号','金额','取数字段','备注'],
...
async def write_result_sheets(token: str, sm: dict, year_month: str, result: dict,
    ...
    # 财务签核产品表由 settlement_engine 的统一 18 列合同写入。
    # 不在这里预写旧 27 列，避免后续 A:R 覆盖后 S:AA 留下错位公式。
...
async def write_settlement_sheets(token: str, sm: dict, settlement: dict):
    ...
    mapping = {
        ...
        "资产分类明细": settlement.get("asset_rows", []),
