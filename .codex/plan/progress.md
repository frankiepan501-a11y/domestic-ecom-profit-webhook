...
## 2026-08-12 P0 整改与静默初检

| Action | Result | Evidence |
|---|---|---|
| 纠正数据源边界 | 停止抖店/浏览器路径；固定运营附件驱动 | README、handoff、memory candidate |
| 修附件识别与缺口同步 | 抖音 CSV、抖音结算表头、小红书订单、天猫拆分导出、无数据状态均覆盖 | 57 项测试通过 |
| 加财务与归档硬闸 | 有 P0 禁发/禁点财务卡；旧按钮拒绝；总表终态双确认 | `app/card_workflow.py`、`app/company_profit.py` |
| 生产部署 | v0.4.0 精确运行 | commit `16608f5` |
| 单变量补总表鉴权 | 变量数 58→59，无整批覆盖 | Zeabur 回读 |
| 作废旧财务卡 | 16/16 成功，0 失败 | 生产 endpoint 返回 |
| 第一次静默重跑 | 初检完成，但富文本读取误判无历史卡，安全停为 partial | 审计 `cost_gap_card_refresh=partial` |
| 修富文本兼容并重跑 | 1 张原卡更新、7 张旧卡停用、0 缺失/0 失败 | 审计 `cost_gap_card_refresh=patched` |
| 最终回读 | open P0=2；新报表=0；新卡=0；有效原卡已 @ 赵伟俊 | Base + IM readback |

### Remaining Work

| Priority | Item | Owner | Trigger |
|---|---|---|---|
| P0 | 补抖音宝空订单级结算明细（含 6 个指定字段） | 赵伟俊/国内电商运营 | 原卡“打开资料上传页” |
| P0 | 补小红书纷岚订单 `P799684994323319861` 快递单号 | 赵伟俊/国内电商运营 | 同一张原卡 |
| P1 | 补完后点击“提交初检” | 赵伟俊/国内电商运营 | 两项资料已齐 |
