...
- 文件夹归类规则已补店铺别名和天猫费用明细：`宝空/宝宝/POWKONG` 可匹配 `POWKONG旗舰店/宝空店`，`纷岚/FUNLAB/梵乐璞` 可匹配 `纷岚店`；`交易货款/返点积分/光合平台/软件服务费/淘金币/消费券/消费者体验提升计划服务费` 等天猫账单明细归入 `平台费用`。附件展示和合并按文件 basename 去重，避免文件夹前缀导致重复。
