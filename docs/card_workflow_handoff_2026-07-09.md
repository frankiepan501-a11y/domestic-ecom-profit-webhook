...
- 2026-08-12 修复天猫补传商品明细误报：淘宝官方 `ExportItemlList*.csv` / `ExportItemList*.csv` 现在与 `ExportOrderList*`、订单明细一起按子订单合并读取。商品明细里的「商家编码」可与订单文件里的物流字段互补，且跨月结算订单的 ERP SKU 会进入成本查询。真实附件回放确认订单 `5113837045333047045` 已正确识别为 `PK02-S2`，不再误报采购成本缺失。
...
## 2026-08-12 P0 卡片止血与附件识别修复（v0.4.0）

### 问题与根因

- 2026-07 同一月份先后产生 4 个报表版本，每版 4 张平台财务卡，共 16 张旧卡；旧按钮仍可点击，且有资料 P0 时仍可能进入财务确认，操作人无法判断哪张有效。
- 财务确认后的旧逻辑可以直接把国内平台 run 标归档，没有把“公司毛利总表灌入成功”设成归档前置条件。
- 抖音订单 CSV、小红书普通“订单明细”、天猫商品明细与订单明细拆分导出等附件识别规则不完整；抖音主体内部资金转移还可能被误算成平台费用。
- 静默修复沿用完整报表生成入口，会有新建报表或重复发卡风险；正确边界应是只读运营现有附件、重算初检、PATCH 原缺口卡。
- 执行过程中一度错误尝试登录抖店。已纠正：本工作流的数据源固定为运营上传的官方导出附件，AI不登录平台后台取数。

### 本次改动

- `app/card_workflow.py`：旧财务卡可批量原位停用；旧版本按钮服务端拒绝；有开放 P0 时禁止发/处理财务卡；四个平台完成后先调用公司毛利汇总服务，成功才归档；财务决定只写 Base 已存在的单选值。
- `app/parsers.py`、`app/settlement_engine.py`：补齐抖音 CSV、小红书订单明细、天猫拆分文件合并、无数据确认及抖音内部转账排除；宝空结算文件按订单级必需表头逐字段核验。
- `app/task_runner.py`、`app/main.py`：新增 `/profit/rerun-initial-check`，只用现有附件重跑初检，不生成报表、不发新卡；结果只同步 P0 台账并原位更新已有运营缺口卡。
- `app/cost_gap_alert.py`：静默重跑时只 PATCH 最新原卡、停用更旧重复卡；不调用任何发送接口。资料、订单、采购成本和物流缺口仍统一由国内运营闭环。
- `app/company_profit.py`：公司毛利总表灌入与归档成为财务定稿后的强制步骤。

### 验证与剩余风险

- 本地自动测试 57 项通过；覆盖“不生成报表、不发新卡、旧按钮拒绝、P0 禁发卡、总表失败不归档、附件识别、原卡更新和旧卡停用”。
- 生产动作必须按顺序执行：部署 v0.4.0 → 停用 16 张旧财务卡 → 调用静默初检 → 回读原缺口卡与 P0 台账。未完成这四步前不能宣称生产止血完成。
- 宝空现有 `抖音 宝空 结算单.csv` 经表头检查属于报送/税务类汇总，不含订单级核算所需字段。静默重跑后只在原缺口卡中给赵伟俊一条精确补件说明，不新增重复卡。
...
## 2026-08-17 小红书取消订单物流误报与抖音SKU确认（v0.4.1，已发布）

- 症状：小红书订单 `P799684994323319861` 的官方订单导出显示“已取消/售后完成”，没有发货和快递单号；旧结算引擎只要看到“结算入账”且快递单号为空，就误报物流 P0。
- 根因：`process_xhs()` 没有在物流判断前读取订单状态，导致取消订单的结算入账行与退款行虽然金额、佣金和采购成本净额均为 0，仍被要求补运费。
- 修复：订单状态含“已取消/取消订单/交易关闭”、快递单号为空且当月结算收入与退款净额为0时，物流日志明确写“未发货/无需运费”，不生成物流 P0；退款未完整抵消或其他无单号订单仍保持 fail-close。
- 抖音SKU确认：蔡宗佑于2026-08-17确认商品ID `3751006771863486516` 对应 ERP SKU `PK02-S3`。系统仅在官方订单明细商家编码为空时使用该确认映射；订单明细已有编码时仍以官方导出为准，并在报表 `Sources` 页留下“SKU映射确认”记录。
- 回归：新增小红书真实订单号、取消但退款未抵消、抖音商品ID映射三项测试，均按红→绿验证；全量 `60/60` 测试、compileall、`git diff --check` 通过。75附件本地预演 P0=`0`；抖音目标订单带出 `PK02-S3`、采购成本 `170.26元`，小红书目标订单显示结算净额0、无需运费。
- 发布：代码提交 `7fcf59c1f33b47d7e2f1d815844d3b40b3d880e6`；Zeabur deployment `6a82f8fa46afbcef424d7383` 为 RUNNING，`/health version=0.4.1`。
- 生产静默初检：2026-07 返回 `open_p0=0 / calculated_p0=0 / created_report=false / sent_new_card=false`；8张历史缺口卡原位失效，失败0。运行台进入“待AI试算”，本次未生成新报表、未发新财务卡、未推进财务通过或归档。

## 2026-08-17 统一18列产品毛利表旧公式残留修复（v0.4.2）

- 症状：新生成的 `产品毛利_月度/季度` 虽然 A:R 已写入莫莉莉确认的统一18列，但 S:AA 仍保留旧版27列模型的辅助列和错位公式，例如 `=K2-S2`。月度汇总金额和P0闸均正常，但财务打开产品表会看到两套口径，不能送审。
- 根因：`write_result_sheets()` 先向两个财务产品表写入旧27列，`write_settlement_sheets()` 随后只覆盖统一列，没有清除尾部旧列。
- 修复：产品毛利月度/季度表从创建开始就使用统一18列表头；旧模型只保留在内部 `10_毛利结果表`，不再预写财务签核产品表；`settlement_engine` 输出行同步锁定18列。
- 复核补强：规格轴审查发现两项上线前缺口——原测试退款额为0，无法锁住 `Q/(J-K)`；抖音无法追溯的其他费用仍按毛销售额分摊。现已把退款场景写入毛利率回归，并将抖音公共其他费用改为按 SKU 净销售额分摊、最大净销售额 SKU 承接分币尾差，确保产品表 P 列合计与月账单一致。
- 单一列合同：财务产品表创建列数和表头都由 `settlement_engine.PRODUCT_HEADER` 派生，避免以后改列时出现定义漂移。
- 回归：新增“旧27列不得写入财务产品表”“表头严格18列”“输出行严格18列及有退款毛利计算”“公共费用按净销售额分摊并补尾差”测试；发布前全量 `64/64` 测试、compileall 和 `git diff --check` 通过。
- 安全边界：发现残留后未发送财务确认卡；问题版本只作废留痕。修复版本重新生成并核验后，才允许进入财务群终审。
