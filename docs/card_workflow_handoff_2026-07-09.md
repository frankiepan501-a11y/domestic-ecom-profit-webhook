...
- 2026-07-13 财务部反馈“报表和原始资料在云盘哪里找”后，补齐国内电商固定云盘归档：新增 `AI国内电商毛利表` 文件夹，folder token `Vr92fqp41lmUICdEMOgcl9uCnkg`，所有者已转 Frankie；后续新建 `(AI)国内电商毛利表-YYYY/MM` 时通过 `DOMESTIC_ECOM_REPORT_FOLDER_TOKEN` 传入该目录。财务确认卡和生成通知会展示云盘位置、云盘按钮和原始资料落点说明。完整规范见 `docs/profit_report_drive_archive.md`。
