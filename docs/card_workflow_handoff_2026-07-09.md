...
## 2026-08-17 2026-07 正式试算送财务终审

- 正式试算表：`https://u1wpma3xuhr.feishu.cn/sheets/Dt6hsOIL5h9JCAtzVIzcK9aBnib`。本表覆盖天猫、京东、抖音、小红书；淘宝、拼多多已有各自财务终审版，本轮不重复送审。
- 报表核验：财务产品表严格 A:R 18列，S:T 为空；产品月度合计与店铺月度汇总逐店一致；P0开放缺口为0；SKU确认 `3751006771863486516 -> PK02-S3` 已进入抖音成本核算并留来源证据。
- 四平台合计：销售额 `54,538.22` 元、退款 `873.43` 元、净销售额 `53,664.79` 元、毛利润 `29,654.47` 元、毛利率约 `55.3%`。
- 发布证据：代码提交 `7f3589908503ba72784f24b11de83a5ca6c74f78`；Zeabur deployment `6a8300e146afbcef424d7582` 为 RUNNING，`/health version=0.4.2`。
- 财务群真实卡片（App3，财务部工作交流群）：抖音 `om_x100b6704038020a0c2e9c73f5a87df5`、天猫 `om_x100b67040070dd00c0029f0a207c0d7`、小红书 `om_x100b67041efcbca0c4a26c079a362ae`、京东 `om_x100b67041f680930c4f78632e8e25fa`。回读确认4张均为交互卡，标题、期间、结算月口径、报表链接和按钮正确。
- 运行台状态：`待财务确认`；4个平台输出记录均为`待确认`。未代替莫莉莉/财务点击、未灌公司毛利总表、未归档。四平台全部确认后，系统才继续总表灌入与归档；任一退回则进入对应补资料/重算闭环。

## 2026-08-18 财务产品名称统一为 ERP 中文品名（v0.4.3）

- 财务反馈：`产品毛利_月度/季度` 的「中文名称」原来使用平台商品长标题，同一 ERP SKU 会因标题不同拆成多行，莫莉莉无法直接判断采购成本是否正确，也与其他渠道毛利表不一致。
- 根因：结算产品汇总键包含平台标题；天猫、抖音、小红书的产品展示名优先取平台标题；旧 `10_毛利结果表` 在 ERP 品名缺失时也会回退平台标题。
- 修复口径：财务产品表、SKU成本明细和旧结果表均固定显示 ERP 中文品名；平台标题只保留在订单/来源明细。同一平台、店铺、已确认 ERP SKU 汇总为一行；空 ERP SKU 不互相合并；ERP 品名缺失时显示明确缺口，不再静默使用平台标题。
- 7月正式表原位修正：继续使用 `https://u1wpma3xuhr.feishu.cn/sheets/Dt6hsOIL5h9JCAtzVIzcK9aBnib`，不新建报表、不更换财务确认卡。产品行由55条平台标题行合并为32条财务核对行，涉及24个 ERP SKU，ERP 品名缺口为0；`10_毛利结果表` 另修正5个旧标题。
- 金额安全：正式写入前逐列核对 H:Q，合并前后销售、退款、平台费、广告费、采购成本、尾程、其他成本、毛利润均保持不变；回读销售额 `54,538.22` 元、退款 `873.43` 元。报表口径说明页已增加“中文名称”规则。
- 验证：天猫、抖音、小红书、旧结果表、空 SKU、健康版本和金额守恒回归均覆盖；全量 `68/68` 测试、compileall、`git diff --check` 通过；导出报表公式错误扫描0项，月度/季度与SKU成本明细已做页面检查。
- 流程状态保持：运行台仍为`待财务确认`，4张原财务群确认卡 message_id 和原报表链接不变；未代替财务确认、未灌公司毛利总表、未归档。
- 发布证据：代码提交 `38fa46697e7ac0b9a5d034f9f84df19155d61ac3`；Zeabur deployment `6a83c22d46afbcef424d9a86` 为 RUNNING，commitSHA 精确一致；线上 `/health` 返回 `version=0.4.3`。

## 2026-08-24 2026-06 广告证据闸与天猫预付款修复（v0.4.4）

### 问题与根因

- 天猫 `0947` 广告流水旧逻辑把所有“支出”都计入广告，当月 `897.04` 元付款/套餐包预付被和后续实际消耗重复扣减；正确的 30 笔实际消耗扣款为 `8,494.79` 元。
- 抖音两店在没有广告流水、也没有零投放确认时，旧逻辑仍把广告费静默写为 0，既不生成资料缺口，也不阻止财务收尾。

### 修复与生产动作

- 天猫广告只读取 `收支类型=支出` 且 `交易类型=扣款`；付款、充值、套餐购买等预付不再进入当月广告消耗。
- 抖音广告证据在所有提前返回分支之前检查；只有广告流水或结构化“已确认无数据”记录存在时才允许广告为 0，否则生成 `资料缺口/广告账单`。
- Frankie 已确认抖音宝空、抖音纷岚 2026-06 零投放且平台没有广告报表可导；资料清单记录为 `recvoR33NgMCRP`、`recvoR2Yo8gWUJ`。
- 最终提交 `43c8d58` 已发布；线上 `/health version=0.4.4`，哨兵 `tmall_ad_actual_consumption_only=true`、`douyin_ad_evidence_gate=true`。
- 按用户要求静默重跑，不发送消息或卡片。旧报表 `WGfisgszdhl4oDtqcCTcBHITnmf` 保留，新报表 `VS5Isry2ThgP7itNpEOcuLQsnmh` 写入正式索引。

### A/B 与公司汇总

- 全量自动测试 `73/73` 通过；新报表逐字段 A/B `120/120 PASS`。
- 新报表销售 `49,266.27`、净销售 `48,818.96`、毛利 `15,279.83`、报表毛利率 `31.2990%`。
- 新旧毛利差 `997.87` 分为：天猫预付剔除 `897.04`，重跑时当前采购成本台版本带来的成本下降 `100.83`；后者不是本次广告代码修复效果。
- 公司总表继续使用原 file token，飞书旧版本保留；最终 6 月销售 `2,301,018.08`、毛利 `234,212.16`、毛利率 `10.1786%`、净利 `6,697.67`、净利率 `0.2911%`。

### 剩余风险

- 本次领星实时成本接口返回 `'data'` 异常，生成器按既有规则使用当前财务采购成本台；A/B 与实际成本行全部通过，不影响本次 P0 收口，但应列为 P1 监控项。
- 若以后要求历史月份完全可重现，生成报表时需保存成本台版本快照，避免当前成本变更被误认为代码修复效果。

## 2026-09-18 v0.4.5 天猫 SKU 毛利汇总

- 新报表增加独立 `SKU毛利汇总`（27 列），仅生成天猫店铺行；`产品毛利_月度/季度` 18 列合同保持原样。天猫平台费和广告按店铺 SKU 净销售额占比分到分，尾差归净销售额最高 SKU；金额不平时阻断报表生成。
- `口径说明`补充：退款折算数量是退款金额比例折算，不等于实际退回仓库件数；尾程是发给消费者的运单物流费用，不含头程和仓储。
- 本地回放 2026-08 官方结算、订单、广告、平台费、物流及已确认补充：21 个 SKU，原始字段与财务 v3 逐 SKU 差异 0；两店 10 项金额与月度和人工基准均差 0.00 元，派生 11 列差异 0。完整回归 79 项通过。对账脚本和结果在 `.codex/plan/tmall-2026-08-rerun-20260916/`。
- 发布前状态：仅本地代码；正式 8 月报表索引、公司汇总和财务终审状态未更新。生产部署后须以 `/health` 的 `version=0.4.5` 与 `tmall_sku_summary_27_columns=true` 确认，不能只看 HTTP 200。


## 2026-09-22 v0.4.6 天猫退款成本证据闸

- 官方退款订单导出新增读取「部分退款/全额退款」、「退款状态」、「货物状态」和「退货物流信息」。
- 采购成本只对「全额退款+退款成功+已寄回且已签收」或「全额退款+退款成功+未发货」抵减整件；部分退款不抵减整件采购成本。退款折算数量仅用于展示。
- 同一订单同时存在商品全额退款和运费部分退款时，全额退款证据优先，不重复冲减成本。
- 用户可见字段与缺口文案统一为「物流运费」；读取历史报表时仍兼容旧表头「尾程费用」。
- 真实8月官方退款表自动识别3笔有效全额退款证据；82项回归、compileall、8月A/B对账和621个线上SKU单元格复核通过，差异0。
- 线上发布以 /health version=0.4.6 与 tmall_refund_cost_evidence_gate=true 为验收信号。
