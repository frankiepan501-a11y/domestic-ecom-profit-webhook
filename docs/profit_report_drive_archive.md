...
2026-08-12 初检创建报表时发现国内电商目录返回 `1061004 forbidden`。已用目录所有者的用户身份把聪哥1号应用 `cli_a9f6ae86fce8dbd8` 重新加为 `full_access` 协作者，并由同一应用身份复验目录读取成功。该应用权限是国内电商报表自动归档的运行前提；如后续再次转移目录所有权或调整协作者，必须保留此应用权限。
