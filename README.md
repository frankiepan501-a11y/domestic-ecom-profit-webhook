...
| COST_GAP_ALERT_FRANKIE_ONLY | true | 新“运营资料缺口/采购成本缺口”分流卡上线闸；Frankie 确认样卡前保持 `true`，确认后生产显式设为 `false` |
| COST_TABLE_URL | 产品采购成本台 URL | 采购成本缺口卡的处理入口 |
...
- 订单未匹配、订单明细没有商家编码/外部系统编号、同一订单出现多个商家编码：归国内运营；卡片发国内电商平台沟通群并 @ 当前岗位。
- ERP SKU 已识别，但产品采购成本台、ERP 成本和领星 `cg_price` 仍为 0：归采购专员。
- 未命中上述确定性规则的采购成本问题不发给业务部门，只私聊 Frankie 留给财务/系统排查。
...
- 单个群或收件人发送失败不会中断其他责任路由，且失败项不记为已发送，后续可重试。
- 防重复记录查询临时失败时，只跳过当前这一张卡并记录告警，其他责任路线和收件人继续发送，避免一次查询异常拖住整批提醒。
- 生产切换前必须保持 `COST_GAP_ALERT_FRANKIE_ONLY=true`，先由 Frankie 验证两类样卡。
